...
int main()
{
...
#endif
  {
    ...
    std::string floating_interval_ = std::move(inputs.front()); inputs.pop_front();
    std::istringstream floating_interval__(floating_interval_); int floating_interval;
    if (!(floating_interval__ >> floating_interval)) { break; }
    ...
    cryptolens::optional<cryptolens::LicenseKey> license_key =
      cryptolens_handle.activate_floating(e, access_token, product_id, key_string, floating_interval);
